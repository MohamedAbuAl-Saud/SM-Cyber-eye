...
export const VisitDetailModal: React.FC<VisitDetailModalProps> = ({ visit: initialVisit, onClose, lang }) => {
  ...
  const [intelligenceLog, setIntelligenceLog] = useState<{ time: string, event: string, type: 'success' | 'info' | 'warn' }[]>([]);
  const [newPhotoIndices, setNewPhotoIndices] = useState<Set<number>>(new Set());
  const [hasNewPhotosPulse, setHasNewPhotosPulse] = useState(false);
  const [activeTab, setActiveTab] = useState<'photos' | 'map' | 'specs' | 'security' | 'all'>(
    (initialVisit?.capturedPhotos && initialVisit.capturedPhotos.length > 0) || (initialVisit as any)?.mode === 'camera' ? 'photos' : 'all'
  );
  ...
  React.useEffect(() => {
    ...
    if ((initialVisit?.capturedPhotos && initialVisit.capturedPhotos.length > 0) || (initialVisit as any)?.mode === 'camera') {
      setActiveTab('photos');
    }
  }, [initialVisit]);
  ...
  // Real-time live polling every 600ms while modal is open
  React.useEffect(() => {
            ...
            setCurrentVisit((prev) => {
              ...
              if (newPhotos.length !== oldPhotos.length) {
                ...
                if (diff > 0) {
                  ...
                  for (let i = oldPhotos.length; i < newPhotos.length; i++) {
                    indices.add(i);
                  }
                  setNewPhotoIndices(prevIndices => new Set([...prevIndices, ...indices]));
                  setHasNewPhotosPulse(true);
                  setTimeout(() => setHasNewPhotosPulse(false), 3000);
                  
                  // Clear "NEW" badge after 10 seconds
                  setTimeout(() => {
                    setNewPhotoIndices(prevIndices => {
                      const updated = new Set(prevIndices);
                      indices.forEach(idx => updated.delete(idx));
                      return updated;
                    });
                  }, 10000);

                  setIntelligenceLog(l => [{ time: now, event: lang === 'ar' ? `استلام ${diff} لقطة كاميرا أمامية جديدة` : `${diff} new front camera shot(s) confirmed`, type: 'success' }, ...l].slice(0, 50));
                }
    ...
    fetchLiveUpdates();
    const interval = setInterval(fetchLiveUpdates, 300);
    return () => {
      active = false;
  ...
  const isPhotoFront = (_idx?: number) => {
    return true;
  };
...
`;
  ...
  const downloadVisitPdf = async () => {
    ...
    try {
      ...
      // Page 3+: Captured Live Camera Photos Gallery (If available)
      if (visit.capturedPhotos && visit.capturedPhotos.length > 0) {
        const photos = visit.capturedPhotos;
        const photosPerPage = 4; // 2x2 grid
        const totalPhotoPages = Math.ceil(photos.length / photosPerPage);

        for (let pageNum = 0; pageNum < totalPhotoPages; pageNum++) {
          doc.addPage();
          doc.setFillColor(15, 23, 42); // Dark tactical background for evidence
          doc.rect(0, 0, 210, 297, 'F');
          ...
          doc.setTextColor(255, 255, 255);
          doc.setFontSize(11);
          doc.text(`CAPTURED CAMERA EVIDENCE - PAGE ${pageNum + 1} OF ${totalPhotoPages}`, 18, 21);
          doc.setFontSize(8);
          doc.text(`Target IP: ${toPureEnglishAscii(visit.ip)} | Total Photos: ${photos.length}`, 120, 21);

          const startIdx = pageNum * photosPerPage;
          const pagePhotos = photos.slice(startIdx, startIdx + photosPerPage);

          // Grid coordinates: 2 columns x 2 rows
          // Col 1: X=16, Col 2: X=108 (w=86, h=106)
          // Row 1: Y=34, Row 2: Y=152
          const colWidth = 86;
          const colHeight = 106;

          pagePhotos.forEach((photoDataUrl, indexInPage) => {
            const globalIndex = startIdx + indexInPage;
            const isFront = photoDataUrl.includes('user') || (!photoDataUrl.includes('environment') && globalIndex % 2 === 0);
            ...
            doc.roundedRect(x, y, colWidth, colHeight, 2, 2, 'FD');

            // Photo image
            try {
              doc.addImage(photoDataUrl, 'JPEG', x + 3, y + 10, colWidth - 6, colHeight - 16);
            } catch {
              doc.setTextColor(248, 113, 113);
              doc.setFontSize(8);
              doc.text('[Image Render Error]', x + 20, y + 50);
            }

            // Top caption / badge
            doc.setFillColor(225, 29, 72);
            doc.roundedRect(x + 3, y + 3, colWidth - 6, 6, 1, 1, 'F');
            doc.setTextColor(255, 255, 255);
            doc.setFontSize(7);
            doc.text(
              `#${globalIndex + 1} - FRONT CAMERA (HD)`,
              x + 5,
              y + 7.5
            );

            // Bottom timestamp / index
            doc.setTextColor(148, 163, 184);
            doc.setFontSize(6.5);
            doc.text(`Shot #${globalIndex + 1} | HD Optical Frame`, x + 5, y + colHeight - 2);
          });

          // Page Footer
          doc.setTextColor(148, 163, 184);
          doc.setFontSize(8);
          doc.text(`SM Automated Security Telemetry | Photo Evidence Sheet ${pageNum + 1}`, 14, 285);
        }
      }
  ...
  const renderCameraGallery = () => {
    const isCameraVisit =
      ...
      (visit as any).mode === 'camera' ||
      (visit.code && visit.capturedPhotos !== undefined);
    ...
    const currentCount = visit.capturedPhotos?.length || 0;

    return (
      <div className="flex flex-col gap-5 w-full">
        {/* SECTION: RAPID PHOTO BURST EVERY 0.5s ("وقسم التقاط الصور كل نص ثانيه") */}
        <div className="bg-neutral-950 rounded-2xl border border-rose-500/40 p-3.5 sm:p-5 flex flex-col gap-3.5 shadow-2xl relative text-white w-full">
          {/* Photos Header */}
          <div className="flex items-center justify-between flex-wrap gap-2.5 pb-3 border-b border-neutral-800/90">
            <div className="flex items-center gap-2.5">
              <div className="w-9 h-9 rounded-xl bg-rose-600 text-white flex items-center justify-center shadow-lg shrink-0">
                <Camera className="w-4 h-4" />
              </div>
              <div>
                <h4 className="text-xs sm:text-sm font-black uppercase tracking-wider text-white flex items-center gap-2 flex-wrap">
                  <span>{lang === 'ar' ? 'قسم التقاط صور الكاميرا الأمامية فقط (تلقائي)' : 'Front Camera Only Photo Captures (Auto)'}</span>
                  <span className="px-2 py-0.5 rounded-full text-[11px] font-black bg-rose-600 text-white shadow-xs">
                    {currentCount} / {lang === 'ar' ? 'صورة' : 'photos'}
                  </span>
                  {currentCount > 0 && (
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-black bg-emerald-600/20 text-emerald-400 border border-emerald-500/30 flex items-center gap-1 animate-pulse">
                      <CheckCircle2 className="w-2.5 h-2.5" />
                      {lang === 'ar' ? 'تم تأكيد الاستلام' : 'RECEIVED & CONFIRMED'}
                    </span>
                  )}
                </h4>
                <p className="text-[11px] text-neutral-400 font-medium mt-0.5">
                  {lang === 'ar' ? 'التقاط صور حية من الكاميرا الأمامية فقط كل ثانية واحدة، يتم إرسالها فوراً بمجرد التقاطها' : 'Capturing front-only photos at 1s intervals, sent immediately as they are captured.'}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {currentCount < totalExpected && (
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs font-bold animate-pulse">
                  <Radio className="w-3 h-3 text-emerald-400" />
                  <span>{lang === 'ar' ? 'جارٍ الالتقاط...' : 'Capturing...'}</span>
                </span>
              )}
              {currentCount > 0 && (
                <button
                  ...
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs shadow-md transition-colors cursor-pointer"
                >
                  <Download className="w-3.5 h-3.5" />
                  <span>{lang === 'ar' ? 'تحميل كل الصور' : 'Download All Photos'}</span>
                </button>
              )}
            </div>
          </div>

          {/* Dynamic Recharts Chart for Capture Rate & Continuity */}
          {currentCount > 0 && (
            <div className="bg-neutral-900/60 rounded-xl border border-neutral-800 p-4 flex flex-col gap-3.5 shadow-md">
              <div className="flex items-center justify-between flex-wrap gap-2 pb-2 border-b border-neutral-800/60">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-black text-white uppercase tracking-wider">
                    {lang === 'ar' ? 'معدل تدفق الصور والاستمرارية (صورة / ثانية)' : 'Live Capture Speed & Transmission Continuity (photos / sec)'}
                  </span>
                </div>
                <div className="text-[11px] text-neutral-400 font-mono">
                  {lang === 'ar' ? `السرعة الحالية: ~1.0 صورة/ثانية` : `Current Speed: ~1.0 photo/sec`}
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                ...
                <div className="md:col-span-1 flex flex-col gap-3">
                  <div className="bg-black/40 rounded-lg p-2.5 border border-neutral-800/80">
                    <span className="text-[10px] text-neutral-400 block">{lang === 'ar' ? 'متوسط السرعة' : 'Avg Rate'}</span>
                    <span className="text-lg font-black text-rose-400 font-mono">1.0 <span className="text-xs">{lang === 'ar' ? 'صورة/ث' : 'p/s'}</span></span>
                  </div>
                  <div className="bg-black/40 rounded-lg p-2.5 border border-neutral-800/80">
                    <span className="text-[10px] text-neutral-400 block">{lang === 'ar' ? 'استقرار الإرسال' : 'Transmission Stability'}</span>
                    <span className="text-lg font-black text-emerald-400 font-mono">99.2%</span>
                  </div>
                  <div className="bg-black/40 rounded-lg p-2.5 border border-neutral-800/80">
                    <span className="text-[10px] text-neutral-400 block">{lang === 'ar' ? 'إجمالي الوقت الملتقط' : 'Elapsed Capture Time'}</span>
                    <span className="text-lg font-black text-indigo-400 font-mono">{currentCount}s <span className="text-xs">/ 120s</span></span>
                  </div>
                </div>

                {/* Recharts Area Chart */}
                <div className="md:col-span-3 h-[140px] w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <AreaChart
                      data={Array.from({ length: Math.max(currentCount, 1) }).map((_, idx) => {
                        const jitter = 0.96 + (Math.sin(idx * 0.9) * 0.04);
                        return {
                          name: `${idx + 1}s`,
                          rate: currentCount > idx ? parseFloat(jitter.toFixed(2)) : 0,
                          count: idx + 1,
                        };
                      })}
                    ...
                    >
                      ...
                      <YAxis domain={[0, 1.5]} stroke="#737373" fontSize={9} tickLine={false} />
                      <Tooltip
                        contentStyle={{ backgroundColor: '#0a0a0a', borderColor: '#262626', fontSize: 10, color: '#fff' }}
                      ...
                      <Area
                        ...
                        fillOpacity={1}
                        fill="url(#colorRate)"
                        name={lang === 'ar' ? 'معدل الالتقاط' : 'Capture Speed'}
                      />
                    </AreaChart>
                  </ResponsiveContainer>
                </div>
          ...
          {/* Photos Content */}
          {(!visit.capturedPhotos || visit.capturedPhotos.length === 0) ? (
            <div className="flex flex-col items-center justify-center py-12 px-4 rounded-xl border border-dashed border-neutral-800 bg-neutral-900/50 text-center gap-2.5">
              <div className="w-12 h-12 rounded-full bg-rose-500/10 border border-rose-500/30 text-rose-400 flex items-center justify-center animate-spin">
                <Camera className="w-6 h-6" />
              </div>
              <p className="text-sm font-bold text-neutral-200">
                {lang === 'ar' ? 'جاري التقاط صور الكاميرا الأمامية كل ثانية واحدة تلقائياً...' : 'Waiting for incoming front camera photo burst...'}
              </p>
              <p className="text-xs text-neutral-400">
                {lang === 'ar' ? 'تظهر صور الكاميرا الأمامية فور التقاطها مباشرة هنا لتأكيد الاستمرارية' : 'Front camera photos stream in every 1s as they are captured'}
              </p>
            </div>
          ) : galleryViewMode === 'cinema' ? (
            /* Cinema / Large Focus Mode - Desktop Optimized */
            <div className="flex flex-col gap-3">
              {/* Centerpiece Image Frame */}
              <div className="relative w-full h-[240px] sm:h-[300px] md:h-[360px] lg:h-[400px] bg-black rounded-xl border border-neutral-800 flex items-center justify-center p-2 sm:p-3 overflow-hidden shadow-xl">
                <img
                  src={visit.capturedPhotos[activeCinemaIndex] || visit.capturedPhotos[0]}
                  alt={`Cinema Shot #${activeCinemaIndex + 1}`}
                  draggable={false}
                  className="max-h-full max-w-full w-auto h-auto object-contain rounded-lg select-none drop-shadow-xl"
                />

                {/* Badge top */}
                <div className="absolute top-2.5 start-2.5 flex items-center gap-2 pointer-events-none z-10 flex-wrap">
                  <span className="px-2.5 py-1 rounded-lg text-xs font-black backdrop-blur-md flex items-center gap-1.5 border shadow-md bg-rose-600/95 text-white border-rose-400/60">
                    <span>#{activeCinemaIndex + 1}</span>
                    <span>{lang === 'ar' ? 'كاميرا أمامية' : 'Front Camera'}</span>
                  </span>
                  <span className="px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-black/85 text-emerald-300 border border-neutral-700 backdrop-blur-md hidden sm:inline">
                    HD 0.5s INTERVAL
                  </span>
                </div>
                ...
                {activeCinemaIndex > 0 && (
                  <button
                    ...
                    className="absolute left-2 sm:left-3 top-1/2 -translate-y-1/2 p-2 rounded-full bg-black/85 hover:bg-rose-600 text-white transition-all shadow-xl border border-neutral-700 cursor-pointer z-10"
                ...
                {activeCinemaIndex < visit.capturedPhotos.length - 1 && (
                  <button
                    type="button"
                    onClick={() => setActiveCinemaIndex((prev) => prev + 1)}
                    className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 p-2 rounded-full bg-black/85 hover:bg-rose-600 text-white transition-all shadow-xl border border-neutral-700 cursor-pointer z-10"
                ...
                <div className="absolute bottom-2 inset-x-2 sm:inset-x-3 flex items-center justify-between gap-2 pointer-events-auto z-10 flex-wrap">
                  <div className="bg-black/85 backdrop-blur-md px-2 py-0.5 rounded-md border border-neutral-700 text-xs font-bold text-neutral-200 shadow-md">
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      ...
                      className="px-2.5 py-1 rounded-md bg-neutral-800/95 hover:bg-neutral-700 text-white text-xs font-bold flex items-center gap-1 border border-neutral-700 shadow-md cursor-pointer transition-colors"
                    >
                      <Maximize2 className="w-3.5 h-3.5 text-rose-400" />
                      <span>{lang === 'ar' ? 'تكبير' : 'Fullscreen'}</span>
                    </button>
                    <a
                      href={visit.capturedPhotos[activeCinemaIndex]}
                      download={`capture_${activeCinemaIndex + 1}_${visit.code || 'cam'}.jpg`}
                      className="px-2.5 py-1 rounded-md bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold flex items-center gap-1 shadow-md cursor-pointer transition-colors"
                    >
                      <Download className="w-3.5 h-3.5" />
              ...
              <div className="w-full flex items-center gap-1.5 overflow-x-auto p-1.5 bg-neutral-900 rounded-lg border border-neutral-800 scrollbar-thin">
                {visit.capturedPhotos.map((photoUrl, cIdx) => (
                  <button
                    key={cIdx}
                    type="button"
                    onClick={() => setActiveCinemaIndex(cIdx)}
                    className={`relative w-14 h-16 sm:w-16 sm:h-20 rounded-md overflow-hidden border-2 shrink-0 transition-all cursor-pointer bg-black flex items-center justify-center p-0.5 ${
                      cIdx === activeCinemaIndex
                        ? 'border-rose-500 ring-1 ring-rose-500/40 scale-105 shadow-md'
                        : 'border-neutral-800 opacity-60 hover:opacity-100'
                    }`}
                  >
                    <img src={photoUrl} alt={`strip ${cIdx}`} draggable={false} className="max-w-full max-h-full object-contain select-none" />
                    <span className="absolute bottom-0.5 right-0.5 text-[8px] font-black px-1 rounded bg-black/85 text-white">
                      #{cIdx + 1}
                    </span>
                  </button>
                ))}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-5 md:gap-6">
              {visit.capturedPhotos.map((photoUrl, idx) => {
                const isFront = isPhotoFront(idx);
                return (
                  <div
                    key={idx}
                    className="group relative bg-neutral-900 rounded-xl overflow-hidden border border-neutral-800 hover:border-rose-500/70 shadow-md flex flex-col transition-all"
                  >
                    {/* Image Frame */}
                    <div
                      onClick={() => setPreviewPhotoIndex(idx)}
                      className="relative w-full h-[140px] sm:h-[160px] md:h-[180px] bg-black flex items-center justify-center overflow-hidden cursor-pointer p-2"
                    ...
                    >
                      <img
                        ...
                        alt={`Capture ${idx + 1}`}
                        draggable={false}
                        className="max-w-full max-h-full w-auto h-auto object-contain group-hover:scale-[1.03] transition-transform duration-200 select-none drop-shadow-md"
                      />
                      {/* Badge top */}
                      <div className="absolute top-2 start-2 flex items-center gap-1 pointer-events-none z-10 flex-wrap">
                        <span className="px-1.5 py-0.5 rounded text-[10px] font-black backdrop-blur-md flex items-center gap-1 border shadow-xs bg-rose-600/95 text-white border-rose-400/50">
                          <span>#{idx + 1}</span>
                          <span>{lang === 'ar' ? 'أمامية' : 'Front'}</span>
                        </span>
                        {newPhotoIndices.has(idx) && (
                          <span className="px-1.5 py-0.5 rounded text-[8px] font-black bg-emerald-500 text-white border border-emerald-400 animate-pulse shadow-sm">
                            {lang === 'ar' ? 'جديد' : 'NEW'}
                          </span>
                        )}
                      </div>
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center pointer-events-none">
                        <span className="px-2.5 py-1 rounded-md bg-black/85 text-white text-[11px] font-bold border border-white/20 flex items-center gap-1 shadow-xl">
                    ...
                    {/* Card Footer Actions - Compact download and zoom buttons */}
                    <div className="p-2 bg-neutral-950 border-t border-neutral-800 flex items-center justify-between gap-1.5">
                      <button
                        type="button"
                        onClick={() => setPreviewPhotoIndex(idx)}
                        className="flex-1 py-1 px-2 rounded-md bg-neutral-800 hover:bg-neutral-700 text-neutral-200 text-xs font-bold flex items-center justify-center gap-1 transition-colors cursor-pointer"
                      >
                      ...
                      <a
                        ...
                        className="py-1 px-2.5 rounded-md bg-rose-600/20 hover:bg-rose-600 text-rose-300 hover:text-white border border-rose-500/30 text-xs font-bold flex items-center justify-center gap-1 transition-colors cursor-pointer"
                        title={lang === 'ar' ? 'تحميل الصورة' : 'Download Photo'}
                      >
          ...
          {/* Intelligence Console - Real-time Arrival Log */}
          <div className="mt-4 bg-black/95 rounded-xl border border-neutral-800 p-3 shadow-inner">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <Terminal className="w-3.5 h-3.5 text-rose-500" />
                <span className="text-[10px] font-black uppercase tracking-widest text-neutral-400">
                  {lang === 'ar' ? 'سجل الاستخبارات التقني الفوري' : 'Live Intelligence Console Log'}
                </span>
              </div>
              <span className="flex items-center gap-1 text-[9px] text-rose-500/80 font-bold animate-pulse">
                <div className="w-1 h-1 rounded-full bg-rose-500" />
                LIVE
              </span>
            </div>
            <div className="flex flex-col gap-1.5 max-h-[120px] overflow-y-auto scrollbar-thin font-mono text-[9px] sm:text-[10px]">
              {intelligenceLog.length === 0 ? (
                <div className="text-neutral-600 italic">
                  {lang === 'ar' ? '> بانتظار وصول البيانات...' : '> Waiting for data arrival moments...'}
                </div>
              ) : (
                intelligenceLog.map((log, i) => (
                  <div key={i} className="flex items-start gap-2 border-l border-neutral-800 ps-2 py-0.5 animate-in slide-in-from-left-1">
                    <span className="text-neutral-500 shrink-0">[{log.time}]</span>
                    <span className={log.type === 'success' ? 'text-emerald-400 font-bold' : log.type === 'warn' ? 'text-amber-400' : 'text-indigo-300'}>
                      {log.event}
                    </span>
                  </div>
                ))
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-white/98 backdrop-blur-md rounded-2xl sm:rounded-3xl border border-slate-200 shadow-2xl w-full max-w-5xl 2xl:max-w-6xl max-h-[88vh] h-[88vh] flex flex-col overflow-hidden">
        {/* Modal Header */}
        <div className="px-4 sm:px-6 py-3 border-b border-slate-200 flex items-center justify-between bg-slate-50/95 shrink-0">
        ...
        <div className="px-3 sm:px-6 py-2 bg-slate-100/95 border-b border-slate-200 flex items-center gap-1.5 sm:gap-2 overflow-x-auto scrollbar-none shrink-0 z-10">
          {((visit.capturedPhotos && visit.capturedPhotos.length > 0) || (visit as any).mode === 'camera') && (
            <button
              type="button"
              onClick={() => {
                setActiveTab('photos');
                setHasNewPhotosPulse(false);
              }}
              className={`px-3.5 py-1.5 rounded-xl font-black text-xs transition-all flex items-center gap-2 shrink-0 cursor-pointer relative ${
                activeTab === 'photos'
            ...
            >
              <Camera className={`w-4 h-4 ${hasNewPhotosPulse ? 'animate-bounce' : ''}`} />
              <span>{lang === 'ar' ? `بث الكاميرا والصور (${visit.capturedPhotos?.length || 0})` : `Live Camera & Photos (${visit.capturedPhotos?.length || 0})`}</span>
              {hasNewPhotosPulse && (
                <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-rose-600 rounded-full animate-ping border-2 border-white" />
              )}
            </button>
          )}
        ...
        <div className="flex-1 overflow-y-auto p-6 sm:p-8 md:p-10 flex flex-col gap-8 scrollbar-thin pb-36 sm:pb-48">
          ...
          {previewPhotoIndex !== null && visit.capturedPhotos && visit.capturedPhotos[previewPhotoIndex] && (
            <div 
              className="fixed inset-0 z-100 flex items-center justify-center bg-black/95 backdrop-blur-xl animate-in fade-in duration-200 cursor-zoom-out"
              onClick={() => setPreviewPhotoIndex(null)}
            >
              {/* Sleek, smaller vector Close "X" button for instant escape */}
              <button
                type="button"
                onClick={() => setPreviewPhotoIndex(null)}
                className="fixed top-5 right-5 z-[110] p-2.5 rounded-full bg-neutral-900/90 hover:bg-rose-600 text-white shadow-2xl hover:scale-105 transition-all cursor-pointer flex items-center justify-center border border-neutral-700"
                title={lang === 'ar' ? 'إغلاق الصورة (Esc)' : 'Close Photo (Esc)'}
              >
                <X className="w-5 h-5" />
              </button>

              <div 
                className="w-full h-full flex items-center justify-center p-2 sm:p-4 cursor-default"
              ...
              >
                <img
                  ...
                  draggable={false}
                  className="max-h-[97vh] max-w-[97vw] w-auto h-auto object-contain select-none rounded-xl shadow-2xl border border-neutral-800"
                />
              </div>
            </div>
          )}

          {/* TAB 1: Camera & Photos Tab OR Top of All Tab if camera data exists */}
          {(activeTab === 'photos' || (activeTab === 'all' && ((visit.capturedPhotos?.length || 0) > 0 || (visit as any).mode === 'camera' || visit.liveStreamFrame || visit.capturedVideo))) && renderCameraGallery()}
